...
class TaskCreatorServer(Server):
    ...
    def _cmd_action(self, cmd: list, clientsocket: socket.socket):
        if self.task:
            if len(cmd) == 1:
                raise Exception("Args num incorrect")

            self.task._prevstage.config["actions"].append(' '.join(cmd[1::]))
    ...
    def _handle_cmd(self, cmd: list, clientsocket: socket.socket):
        if cmd[0] == "start":
            ...
            self._cmd_action(cmd, clientsocket)
