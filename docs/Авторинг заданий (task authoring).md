...
| `deny` | `deny <база…>` | Политика: этими командами стадию НЕ засчитать (реши иначе). Взаимоисключимо с `allow`. |
| `allow` | `allow <база…>` | Политика: засчитывается ТОЛЬКО этими командами (напр. не `echo`-подстава). Проверяется только у команды, которая иначе прошла бы — навигация не блокируется. |
...
|---|---|---|
| `say "..."` | `say "Йо! Приручаем логи 🧑‍💻"` | Показать текст (печатающий вывод). |
...
settings
  ...
  type-speed 45              # символов/сек
  pager on                   # большие show file → пейджер

voice
  hello  say "Йо! Сегодня приручаем логи 🧑‍💻"     # при входе (первая стадия)
  bye    say "Красава 🎉"                          # когда все стадии пройдены
