...
class Stage():
    ...
    def __init__(self, config_file: str, mode='Interactive', fs_analyzer=True, fs_accuracy=0.7, output_analyzer=True,
                 output_accuracy=0.7,
                 observe_dirs=None):
        if observe_dirs is None:
            observe_dirs = ['/home/anton/LinuxWork/study/8/nirs/', '/tmp/', '/usr/bin/', '/usr/local/bin', '/etc/']
        ...
        self.config['options']['observe_dirs'] = observe_dirs
    ...
    def start(self):
        for dir in self.config['options']['observe_dirs']:
            self._observe_processes.append(self._observe_dir(dir))
