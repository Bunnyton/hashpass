...
spawn systemd-nspawn -b -q \
	-D ./test \
	--user root # \
	# sudo script -qfc 'sudo -u student bash -i /usr/bin/hash' /var/log/hashpass.log
...
expect "root@" { send "script -fc 'sudo -u student bash -i /usr/bin/hash' /var/log/hashpass.log; shutdown now\r" }
expect "@" { send "clear\r" }
