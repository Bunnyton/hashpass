...
class TaskCheckerServer(Server):
    ...
    def check_change(self, path) -> bool:
        ...
        if path == 'output':
            ...
            data = readfile(self._cmd_output_file)
            data = filter_hook(self._cmd, data, self.curstage_num)
    ...
    def check(self) -> list | None:
        res = self.check_stage()
        if not self.is_completed and res is not None:
            self.curstage_num += 1
            if self.curstage_num >= self.config['stage_amount']:
                self.is_completed = True
                res.append(self.task_complete())
    ...
    def _handle_cmd(self, cmd: list, clientsocket: socket.socket):
        if len(cmd) == 1 and cmd[0] == 'check':
            ...
            res = self.check()
            if res is not None: 
                self.reply(json.dumps(res), clientsocket)


        if len(cmd) > 1 and cmd[0] == 'cmd':
            self._cmd = ' '.join(cmd[1::])
            cmds = cmd_hook(self._cmd, self.curstage_num)
