...
def test_add_with_profile_and_role(tmp_path):
    ...
    us.add("bob", "pw", role="student", full_name="Боб Иванов", group="ИУ7-11", comment="note")
    assert us.verify("bob", "pw")
    assert not us.verify("bob", "nope")
    prof = us.get("bob")
    assert prof == {"user": "bob", "role": "student", "full_name": "Боб Иванов",
                    "group": "ИУ7-11", "comment": "note", "created_at": prof["created_at"]}
...
def test_legacy_string_record_normalizes(tmp_path):
    ...
    assert us.get("old")["full_name"] == ""


@pytest.mark.tier1
def test_all_users_sorted_profiles_without_hashes(tmp_path):
    us = UserStore(tmp_path / "u.json")
    us.add("zoe", "pw", full_name="Z")
    us.add("amy", "pw", full_name="A")
    users = us.all_users()
